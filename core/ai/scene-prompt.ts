...
export function buildScenePrompt(request: string, board = "", memory = ""): string {
  ...
  return [
    ...
    "",
    "If the request is genuinely ambiguous — you cannot tell what to build, or",
    "which existing nodes it refers to — do not guess. Reply with ONLY:",
    '{"question":{"text":"one short question","options":["a","b"]}}',
    "and no nodes. Ask only when a wrong guess would waste the user's time;",
    "otherwise build something and let them refine it.",
    "",
    "Otherwise reply with ONLY JSON of this shape:",
    "{",
    '  "layout": "flow",',
    '  "nodes": [',
